 ...
 */
contract HEXStrategy is Ownable2Step, ReentrancyGuard {
    ...
    event UpdateDistributionPercentages(
        uint16 __hexPrizePoolPercent,
        uint16 __hexStakingPercent,
    ...
    function claimAndDistributeRewards() public nonReentrant onlyAfter30Days {
        ...
        totalStakeCount = IHEXStaking(_hexTokenAddress).stakeCount(
            address(this)
        );

        uint256 totalStakedForCycle = currentCycle;

        //That means there are no stake so add both the cycles in the total stake amount
        if (totalStakeCount == 0) {
            totalStakedForCycle = totalStakedForCycle + nextCycle;
        }

        //checking if any extra rewards recieved other than staked amount
        if (stakeReturn > totalStakedForCycle) {
            _rewardHEX = stakeReturn - totalStakedForCycle;
        }
    ...
    function inCaseTokensGetStuck(
        ...
        require(_token != _hexTokenAddress, "Can not send HEX tokens");
     ...
     * @param _to is the address to be transferred to
     */
    function transferStuckPLS(uint256 _amount, address _to) external onlyOwner {
        require(_to != address(0), "Not valid address");
        require(_amount > 0, "Can not send 0 PLS");
        require(_amount <= address(this).balance, "Can not send more PLS");
    ...
    function stakeHEX() private {
        ...
        if (_currentTotalStakeAmount < _withdrawQueueTotalAmount) {
            return;
        }
