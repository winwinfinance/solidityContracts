...
contract PrizePool is Ownable2Step, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(uint256 => bool)) public tierVerifiedForNoWinner;
    ...
    function claimPrizes(
        ...
        for (uint256 i = 0; i < _draw.length; ++i) {
            ...
            uint32 endingCycleTime;

            endingCycleTime = uint32(
    ...
    function claimWinning(
        ...
        if (
            _tier > 0 &&
            winnerInfo.tierWinner[validDraw][_tier - 1] == address(0)
        ) {
            require(
                tierVerifiedForNoWinner[validDraw][_tier - 1],
                "verify or claim for previous tier first"
            );
        }
        ...
        uint256 alreadyWinnersLength = _alreadyWinners.length;
        for (uint256 k = 0; k < alreadyWinnersLength; ++k) {
        ...
        if (alreadyWinnersLength > 0) {
            for (uint j = 0; j < alreadyWinnersLength; ++j) {
                if (i >= ((_tier + 1) * 5)) {
                    break;
                }
                Ranges memory m;
    ...
    function verifyNoWinnerForTier(
        uint256 _tier,
        address[] memory _alreadyWinners,
        uint256[] memory _rangeIndexOfWinners
    ) external {
        uint256 validDraw = drawCount - 1;
        require(drawCalled[validDraw], "draw not called");
        require(
        ...
        require(
            winnerInfo.tierWinner[validDraw][_tier] == address(0) &&
                !tierVerifiedForNoWinner[validDraw][_tier],
            "tier already won or verified"
        );

        uint256 alreadyWinnersLength = _alreadyWinners.length;
        require(
            alreadyWinnersLength == _rangeIndexOfWinners.length &&
                alreadyWinnersLength == 5,
            "not valid lengths"
        );
        ...
        _verifyNoWinnerTier(
            ...
            _tier,
            _alreadyWinners,
            _rangeIndexOfWinners
        );
    }

    /**
     * @dev Verifies the information provided for a tier without a winner
     * @param _validDraw the current draw
     * @param _tier the tier without a winner
     * @param _alreadyWinners already winner addresses for the tier
     * @param _rangeIndexOfWinners indexes of winner ranges for the tier
     */
    function _verifyNoWinnerTier(
        uint256 _validDraw,
        uint256 _tier,
        address[] memory _alreadyWinners,
        uint256[] memory _rangeIndexOfWinners
        ...
        for (uint j = 0; j < alreadyWinnersLength; ++j) {
            ...
            if (winningIndex != 0) {
                if (_alreadyWinners[j] == address(0)) {
                    ...
                    m = userRanges[_alreadyWinners[j]][_rangeIndexOfWinners[j]];
                }
            }
        ...
        if (i == ((_tier + 1) * 5)) {
            tierVerifiedForNoWinner[_validDraw][_tier] = true;
        }
    }
    ...
    function removeZerosFromEmptyRanges(uint[] calldata _emptyArray) external {
        uint256 emptyArrayLength = _emptyArray.length;
        for (uint256 i = 0; i < emptyArrayLength; ++i) {
    ...
    function drawRandomNumber(uint256[] calldata _slots) external {
        ...
        _handleShuffle(_slots);
        randomNumberRequestId = IRandomNumber(randomNumberAddress)
            .requestRandomWords();
    ...
    function _setRandomNumbers(uint256 _drawCount) private {
        if (!isRandomNumberSet[_drawCount]) {
            ...
            (fulfilled, randomNumber) = IRandomNumber(randomNumberAddress)
                .getRequestStatus(randomNumberRequestId);
